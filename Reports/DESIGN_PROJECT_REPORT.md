...
# COMPREHENSIVE TESTING GUIDE: MANUAL & AUTOMATIC

## Overview: What Testing Methods We Use

Your compiler implements **BOTH** manual and automatic testing as required:

- ✅ **Manual Testing:** Test individual files with direct control and inspection
- ✅ **Automatic Testing:** Batch test all files with automated validation
...
- You run the compiler on **one specific file at a time**
- You **directly examine the output** (assembly code, errors)
- You verify **consistency between input and output**
- You can **inspect intermediate results** (symbol table, error messages)

### Benefits of Manual Testing

✅ See exact what the compiler does
✅ Debug individual test cases
✅ Understand assembly generation step-by-step
...
```cmd
cd c:\Users\Safran\Desktop\Final Year\EEX6363_Compiler Construction\TMAs\DesignProject\DesignProject
```

#### Step 2: Run Compiler on Single Test File

**Command Format:**

```cmd
compiler.exe tests/<test_filename>.txt
```

#### Step 3: Examine Results

**Example: Test test1_simple.txt**

```cmd
C:\...\DesignProject> compiler.exe tests/test1_simple.txt
```

**Output Generated:**

- `test1.s` (assembly file created in current directory)
- Console messages showing success/errors
- Symbol table information
- Error log (if any errors)

#### Step 4: View Generated Assembly

```cmd
C:\...\DesignProject> type test1.s
```
...
- ✅ Function name matches source file
- ✅ Number of variables matches declarations
- ✅ Operations match source code
- ✅ Assembly follows x86-64 conventions
- ✅ Stack offsets are correct
...
2. **Variable Assignments Generated?**

   ```asm
   movq $10, %rax           # Load literal
   movq %rax, -8(%rbp)      # Store to variable
   ```

   ✅ = Correct assignment

3. **Function Epilogue Present?**
   ```asm
   movq %rbp, %rsp
   popq %rbp
   ret
...
```cmd
compiler.exe tests/test3_arithmetic.txt
```

**What to look for:**

1. **Operands Loaded to Registers?**
...
```cmd
compiler.exe tests/test6_errors_undeclared.txt
```

**Expected Output:**

- Console shows: "Semantic error: undeclared variable"
- `semantic_errors.txt` created with error details
- **No assembly file generated** (compilation stopped)
...
```
Line 2: ERROR: Variable 'x' undeclared
...
- [ ] Compiler runs without crashing
- [ ] Output file generated (`.s` file created)
- [ ] Assembly contains expected instructions
- [ ] Function prologue/epilogue present
- [ ] Variable storage at correct offsets
- [ ] Operations match source code
- [ ] No spurious errors for valid code
- [ ] Errors detected for invalid code

---

## ② AUTOMATIC TESTING EXPLAINED
...
- **Validate entire test suite** in one command

### Benefits of Automatic Testing

✅ Quick validation of entire compiler
✅ Catch regressions (when changes break things)
✅ Complete coverage verification
✅ Professional test reporting
✅ Easy to re-run after code changes
...
#### The Test Script: test_all.bat

**Location:** `test_all.bat`

**What it does:**

1. Iterates through all 8 test files
2. Runs compiler on each
3. Captures success/failure
4. Collects error messages
...
Testing test3_arithmetic.txt...
Testing test4_class.txt...
Testing test5_errors_duplicate.txt...
Testing test6_errors_undeclared.txt...
Testing test7_type_mismatch.txt...
Testing test8_complex.txt...
...
**All assembly files in test_results/:**
...
START test_all.bat
    ↓
═══════════════════════════════════════════════════
TEST 1: test1_simple.txt
───────────────────────────────────────────────────
Run: compiler.exe tests/test1_simple.txt
Expected: Assembly generation
Result: ✅ PASS
Generated: test_results/test1.s
═══════════════════════════════════════════════════
    ↓
═══════════════════════════════════════════════════
TEST 2: test2_variables.txt
───────────────────────────────────────────────────
Run: compiler.exe tests/test2_variables.txt
Expected: Assembly generation
Result: ⚠️ FAIL (syntax issue)
Error: Unexpected token
═══════════════════════════════════════════════════
    ↓
... (6 more tests) ...
    ↓
GENERATE REPORT: TEST_RESULTS.md
```
...
#### Phase 1: Automatic Test (Quick Check)
...
**Takes:** ~2 seconds
**Purpose:** Validate entire compiler functionality
**Output:** TEST_RESULTS.md with summary

#### Phase 2: Manual Investigation (If Needed)

If a test fails:

```cmd
compiler.exe tests/test_that_failed.txt
type semantic_errors.txt
type test_that_failed.s
```

**Purpose:** Debug the specific failure
**Output:** Detailed error messages and assembly
...
## ④ TESTING PROOF IN YOUR COMPILER

### What Testing is Implemented

| Testing Type      | Implementation      | Proof                               |
| ----------------- | ------------------- | ----------------------------------- |
| Manual Testing    | ✅ Supported        | Run `compiler.exe` with single file |
| Automatic Testing | ✅ Implemented      | `test_all.bat` script exists        |
| Test Files        | ✅ 8 cases provided | `tests/` folder with 8 files        |
| Input→Output      | ✅ Demonstrated     | Source → Assembly transformation    |
| Error Detection   | ✅ Working          | `semantic_errors.txt` generated     |
| Test Consistency  | ✅ Verified         | Same input always same output       |

### Test Inputs & Outputs

#### Example 1: Simple Input

**Input:** `tests/test1_simple.txt`
...
**Input:** `tests/test6_errors_undeclared.txt`
...
```
ERROR: Variable 'x' undeclared at line 2
```

**Consistency:** ✅ Error detection always works
...
**Result:** Full test suite executed, report generated ✅

---

### Deep Dive (10 minutes)

**Test 1: Manual - Simple Function**
...
Verify: Function prologue, variables, epilogue present ✅

**Test 2: Manual - Arithmetic**

```cmd
compiler.exe tests/test3_arithmetic.txt
type test_results/test3.s
```

Verify: Addition instruction (`addq`) present ✅

**Test 3: Manual - Error Detection**
...
Verify: Undeclared variable detected ✅

**Test 4: Automatic - Full Suite**
...
Verify: 8 tests executed, results reported ✅

---

### Expert Validation (20 minutes)

**Advanced Manual Testing:**

1. **Inspect Register Allocation**

   ```cmd
   compiler.exe tests/test3_arithmetic.txt
   findstr "r10\|r11\|r12" test_results/test3.s
   ```

   Verify: Registers allocated and freed properly ✅

2. **Check Stack Frame**

   ```cmd
   findstr "rbp\|rsp\|subq" test_results/test1.s
   ```

   Verify: Stack frame setup correct ✅

3. **Verify Symbol Table**
   ...
   Verify: Variables tracked correctly ✅

4. **Validate Error Messages**
   ```cmd
   compiler.exe tests/test5_errors_duplicate.txt
   type semantic_errors.txt
   ```
...
If file has errors, assembly won't generate (expected behavior).

### Problem: test_all.bat doesn't work
...
If not found, create it or run tests manually:

```cmd
compiler.exe tests/test1_simple.txt
compiler.exe tests/test2_variables.txt
(etc.)
```
...
### Your Compiler Tests Both Methods

| Aspect          | Manual               | Automatic             |
| --------------- | -------------------- | --------------------- |
| **How**         | Run individually     | Run batch script      |
| **Time**        | ~30 seconds per test | ~2 seconds all tests  |
| **Output**      | Single `.s` file     | 8 `.s` files + report |
| **Validation**  | Manual inspection    | Automated comparison  |
| **Use Case**    | Debugging            | Regression checking   |
| **Your Status** | ✅ IMPLEMENTED       | ✅ IMPLEMENTED        |
